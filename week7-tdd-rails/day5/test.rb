...
O O O O O O O O
  ...
  def test_can_attack_on_diagonal_opposite_way
    queens = Queens.new(white: [6, 6], black: [1, 1])
    assert queens.attack?
  end

  def test_can_attack_on_diagonal2

    queens = Queens.new(white: [0, 6], black: [1, 7])
    assert queens.attack?
  end

  def test_can_attack_on_diagonal3
    queens = Queens.new(white: [4, 1], black: [6, 3])
    assert queens.attack?
  end

  def test_can_attack_on_other_diagonal
    queens = Queens.new(white: [6, 0], black: [2, 4])
    assert queens.attack?
  end

  def test_can_attack_on_other_diagonal_opposite_way
    queens = Queens.new(white: [0, 3], black: [3, 0])
    assert queens.attack?
  end
