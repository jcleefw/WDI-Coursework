...
var thisArrayValue = thisArray[randomNumber(0, thisArray.length)];
var thatArrayLength = thatArray[randomNumber(0, thatArray.length)];


function whatDoesYourStartupDo () {
  //console.log('So.... your startup is basically like '+ randomThis() + " for " + randomThat() + "?");
  console.log('So.... your startup is basically like '+ thisArrayValue + " for " + thatArrayLength + "?");
}
...
/*function randomThis() {
  ...
  return thisArray[randNo];
}

function randomThat() {
  var length = thatArray.length;
  var randNo = Math.floor(Math.random() * length);
  return thatArray[randNo];
}*/
